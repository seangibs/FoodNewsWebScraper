...
def cfia(
    url, element, urlformat, urlfinder, dateformat
):  # specific logic for this site
    df = pd.read_html(url)[0]
...
def date_list(ws_data, url_l, dateURLelement, datefinder, textlen, sitetype):
    ...
    if sitetype == "NZ_FSA":  # date is stored on next page
        ...
        for i in range(textlen):
            a = website_text(
                url_l[i], dateURLelement
            )  # gets all content from that page
            # print(a)
...
def dictionary(
    ...
    url_l = url_list(
        url, ws_data, urlformat, urlfinder, len(text_l), sitetype
    )  # list of all corresponding urls

    date_l = date_list(
        ws_data, url_l, dateURLelement, datefinder, len(text_l), sitetype
    )  # list of all corresponding dates
...
# GFSI test
dictionary(
    "https://mygfsi.com/news-and-resources/?type=news_updates",  # url
    "#response",  # element
    r"url\(\'(.*?)\'\);",  # urlfinder
...
# #USDAFSIS
dictionary(
    "https://www.fsis.usda.gov/wps/portal/fsis/topics/recalls-and-public-health-alerts/current-recalls-and-alerts",  # url
...
# USDA
dictionary(
    "https://www.usda.gov/media/press-releases",  # url
...
# UK_FSA
dictionary(
    "https://www.food.gov.uk/news-alerts/search/alerts",  # url
...
# cfia
cfia(
    "https://www.inspection.gc.ca/about-the-cfia/newsroom/food-recall-warnings/eng/1299076382077/1299076493846",  # url
...
# #RASFF
ecrasff(
    "https://webgate.ec.europa.eu/rasff-window/portal/index.cfm?event=notificationsList",  # url
...
# # #NZ FSA
dictionary(
    "https://www.mpi.govt.nz/food-safety/food-safety-for-consumers/recalled-food-products/",  # url
...
# # #FSAI FoodAlerts
dictionary(
    "https://www.fsai.ie/news_centre/food_alerts.html",  # url
    "#content",  # element
...
dictionary(
    "https://www.fsai.ie/news_centre/allergen_alerts.html",  # url
    "#content",  # element
    r'<p class="title"><a href="(.*?)">',  # urlfinder
    r'(?:.html|.aspx\?id=\d{5})">(.*?)</a><br/><em class="emp">',  # textfinder
    r'</a><br/><em class="emp">(.*?)</em></p><p>',  # datefinder
...
# #FSANZ
dictionary(
    "https://www.foodstandards.gov.au/industry/foodrecalls/recalls/Pages/default.aspx",  # url
